...
func TestPKCS12CAdESPAADRTv24(t *testing.T) {
	chain := fixtures.NewChain(t)
	p12 := fixtures.NewPKCS12(t, chain, fixtures.DefaultPKCS12Password)
	content := []byte("signer-engine cades icp-brasil PA_AD_RT_v2_4 e2e")
	signingTime := defaultSigningTime
	policyInfo := requirePolicyInfo(t, icpbrasil.PolicyNamePAADRTv24)
	...
	for _, mode := range []signing.Mode{signing.ModeAttached, signing.ModeDetached} {
		t.Run(string(mode), func(t *testing.T) {
			...
			timestampAttr := utils.RequireOnlyUnsignedAttr(t, signerInfo, cades.OIDSignatureTimeStampToken)
