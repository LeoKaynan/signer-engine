...
func TestPKCS12CAdESPAADRV(t *testing.T) {
	chain := fixtures.NewChain(t)
	p12 := fixtures.NewPKCS12(t, chain, fixtures.DefaultPKCS12Password)
	content := []byte("signer-engine cades icp-brasil PA_AD_RV e2e")
	signingTime := fixtures.DefaultSigningTime
	policyInfo := requirePolicyInfo(t, icpbrasil.PolicyNamePAADRV)
