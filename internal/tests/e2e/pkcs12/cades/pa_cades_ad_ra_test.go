...
func TestPKCS12CAdESPAADRA(t *testing.T) {
	chain := fixtures.NewChain(t)
	p12 := fixtures.NewPKCS12(t, chain, fixtures.DefaultPKCS12Password)
	content := []byte("signer-engine cades icp-brasil PA_AD_RA e2e")
	signingTime := fixtures.DefaultSigningTime
	policyInfo := requirePolicyInfo(t, icpbrasil.PolicyNamePAADRA)
	...
	for _, mode := range []signing.Mode{signing.ModeAttached, signing.ModeDetached} {
		t.Run(string(mode), func(t *testing.T) {
			...
			assertUnsignedAttrs(t, signerInfo,
				...
				expectedUnsignedAttr{oid: cades.IdArchiveTimeStampV2, kind: unsignedAttrTimestamp},
			)
